                    ...
                    <form class="edit-contact-item mb-5 mt-5" id="form_gallery" name="form_gallery"> 
                      ...
                      <div class="row">
                       <div class="col s12">
                        ...
                        <div class="input-field col s12">
                          <input id="project_url" name="project_url" type="text" class="validate" value="<?=$project_url?>">
                      ...
                      <div class="input-field col s12 file-box">
                        <span class="avatar-contact avatar-online"><img id="icon_src" style="height:100px;" src="<?=$thumbnail?>" alt="avatar"></span>
                      </div>
                      ...
                      <div class="file-data">
                        ...
                        if ($type=='video') {
                          ?>
                          <div class="file-field input-field col s12 file-input">
                           <div class="btn"> <span>Choose</span> <input type="file" id="video_file" name="video_file" onchange="uploadVideo()">
                            </div>
                            <div class="file-path-wrapper"> <input class="file-path validate" type="text" name="video_name" placeholder="Upload Video"> 
                            </div> 
                         </div> 
                         <?php
                         $video1 = "";
                         $video2 = "";
                         if(strpos($video_url, 'amazonaws.com') !== false || strpos($video_url, 'cloudfront.net') !== false){
                          $video1 = trim($video_url);
                        }
                        else
                        {
                          $video2 = trim($video_url);
                        }
                        ?>
                        <?php
                        if ($video1!="") {
                          echo '<div style="float:right;text-align:right;" class="input-field col s12 file-box-video"><a href="'.$video1.'">'.$video1.'</a>&nbsp;&nbsp;<span style="font-size:24px;color:red;cursor:pointer;" onclick="removeVideo()">x</span></div><input type="hidden" id="video_data" name="video_data" value="'.$video1.'">';
                        }
                        ?>
                            <div class="col s12" style="text-align: center;">--OR--</div> <div class="input-field col s12"> <input id="video_url" name="video_url" type="text" class="validate" value="<?=$video2?>"> <label for="video_url">Video URL</label> <div></div> </div>
                            <input type="hidden" id="video_data" name="video_data" value="">
                          <?php
                        }
                        ?>
                      </div>
...
function editgallery(id,title,project_url,url,type)
{
  $("#id").val(id);
  $("#titleE").val(title);
  $("#project_urlE").val(project_url);
  if (type=='image') {
    var filetype = '<div class="file-field input-field col s12 file-input"> <div class="btn"> <span>Choose</span> <input type="file" id="fileE" name="fileE" onchange="imagesPreview(this,2)"> </div> <div class="file-path-wrapper"> <input class="file-path validate" name="file_nameE" type="text" placeholder="Upload Image"> </div> </div> <div class="input-field col s12 file-box"> <span class="avatar-contact avatar-online"><img id="icon_src" style="height:100px;" src="'+url+'" alt="avatar"></span></div>';
  }
  else
  {
    var filetype = '<div class="file-field input-field col s12 file-input"> <div class="btn"> <span>Choose</span> <input type="file" id="file" name="file" onchange="uploadVideo()"> </div> <div class="file-path-wrapper"> <input class="file-path validate" type="text" name="file_name" placeholder="Upload Video"> </div> </div> <div class="input-field col s12 file-box"> </div> <div class="col s12" style="text-align: center;">--OR--</div> <div class="input-field col s12"> <input id="video_url" name="video_url" type="text" class="validate"> <label for="video_url">Video URL</label> <div id="error_title"></div> </div><input type="hidden" id="video_data" name="video_data">';
  }
...
$("form[name='form_gallery']").validate({
  ...
  submitHandler: function(form, event) {
   ...
   $.ajax({
    url: '<?=base_url()?>klconfig/updateGallery',
    type: 'POST',
