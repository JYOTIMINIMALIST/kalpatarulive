   ...
   function fetch_data($query)
    {
        ...
        $query2 = $this->db->select('properties.*,cities.title as city_name,locations.location as location_name,cities.slug as city_slug,locations.slug as location_slug')
            ->from('properties')
            ->join('cities','cities.id = properties.city')
            ->join('locations', 'locations.id = properties.location', 'left')
            ->like('properties.title',$query)
            ->or_like('`typology`',$query)
            ->or_like('`cities.title`',$query)
            ->or_like('`locations.location`',$query)
            ->group_by(['properties.id','properties.typology', 'cities.title'])
            ->get();
        ...
        if($query2->num_rows() > 0){
            ...
            foreach($query2->result_array() as $row)
            {
                ...
                if(strpos($row["typology"],$query)) {
                    $output[] = array(
                        'name' => $row["typology"]
                    );
                }
                elseif (strpos($row["city_name"],$query))
                {
                    $output[] = array(
                        'name' => $row["city_name"]
                    );
                }
                elseif (strpos($row["location_name"],$query))
                {
                    $output[] = array(
